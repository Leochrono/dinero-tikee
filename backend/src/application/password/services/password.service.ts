  ...
  async changePassword(
    ...
    try {
      ...
      if (user.requiresPasswordChange && user.passwordUpdatedAt) {
        const isExpired = await this.isTemporaryPasswordExpired(
        ...
        if (isExpired) {
          return {
            success: false,
            error:
              'La contraseña temporal ha expirado. Por favor, solicita una nueva.',
          };
        }
      }
      ...
      await this.addToHistory(userId, newPassword, {
        changedBy: 'user',
        reason: user.requiresPasswordChange
          ? 'temporary_change'
          : 'user_request',
        ...metadata,
      });
  ...
  async validateRecoveryCode(
    ...
    try {
      if (!user.requiresPasswordChange) {
        return {
          isValid: false,
          error: 'No hay una solicitud de recuperación activa',
        };
      }
      ...
      return {
        isValid: false,
        error: 'Error al validar código de recuperación',
      };
    }
  }
}

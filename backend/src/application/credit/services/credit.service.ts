...
export class CreditService {
  ...
  async create(createCreditDto: CreateCreditDto & { userId: string }): Promise<CreditResponseDto> {
    try {
  ...
  async search(searchParams: SearchCreditDto) {
    try {
      ...
      if (searchParams.amount) {
        queryBuilder.andWhere('credit.amount <= :amount', { amount: searchParams.amount });
      }
      if (searchParams.term) {
        queryBuilder.andWhere('credit.term <= :term', { term: searchParams.term });
      }
      if (searchParams.type) {
        queryBuilder.andWhere('institution.type = :type', { type: searchParams.type });
      }
  ...
  async updateStatus(
    ...
    try {
      ...
      throw new BadRequestException(error.message || 'Error al actualizar el estado del crédito');
    }
  }
  ...
  private async ensureCreditExists(creditId: string, maxAttempts = 5): Promise<CreditEntity> {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        this.logger.debug(`Attempt ${attempt}/${maxAttempts} to verify credit ${creditId}`);
    ...
    throw new NotFoundException(`No se pudo verificar el crédito ${creditId} después de ${maxAttempts} intentos`);
  }
  ...
  async findAllByUser(userId: string): Promise<CreditEntity[]> {
    try {
      ...
      throw new BadRequestException('Error al obtener los créditos del usuario');
    }
  }
}
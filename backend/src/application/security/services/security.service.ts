...
export class SecurityService {
  ...
  async handleFailedAttempt(
    ...
    try {
      ...
      if (user.failedLoginAttempts >= this.CONFIG.MAX_FAILED_ATTEMPTS) {
        ...
        lockExpiration.setMinutes(
          lockExpiration.getMinutes() + this.CONFIG.LOCK_DURATION,
        );
  ...
  async getSecurityEvents(
    ...
    if (eventType) {
      queryBuilder.andWhere('securityLog.eventType = :eventType', {
        eventType,
      });
    }

    if (startDate) {
      queryBuilder.andWhere('securityLog.timestamp >= :startDate', {
        startDate,
      });
    }

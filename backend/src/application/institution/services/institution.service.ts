...
export class InstitutionService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      if (count === 0) {
        ...
        this.logger.log(
          `Se inicializaron ${institutions.length} instituciones`,
        );
      }
      ...
      this.logger.error(
        'Error durante la inicialización de instituciones:',
        error,
      );
      throw new BadRequestException(
        'No se pudieron inicializar las instituciones',
      );
    }
  }

  async filterInstitutions(
    params: FilterInstitutionsDto,
    ...
    try {
      ...
      this.logger.log(
        `Instituciones encontradas: ${filteredInstitutions.length}`,
      );
      if (filteredInstitutions.length > 0) {
        this.logger.debug(
          'Instituciones que cumplen los criterios:',
      ...
      throw new BadRequestException(
        `Error al filtrar instituciones: ${error.message}`,
      );
    }
  }
  ...
  async findBestRates(
    ...
    try {
      if (amount <= 0 || term <= 0) {
        throw new BadRequestException(
          'Monto y plazo deben ser mayores que cero',
        );
      }

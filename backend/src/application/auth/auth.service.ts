...
export class AuthService {
  ...
  async validateUser(
    ...
    try {
      const { user, error } =
        await this.userService.verifyUserCredentials(email);

      if (error) {
        throw new UnauthorizedException(error);
      }

      const isValidPassword = await this.passwordService.validateUserPassword(
        user,
        password,
      );
      if (!isValidPassword) {
  ...
  async register(userData: any) {
    try {
      const validation = await this.passwordService.validatePasswordStrength(
        userData.password,
      );
  ...
  async recoverPassword(email: string, ipAddress?: string, userAgent?: string) {
    try {
      ...
      const result = await this.passwordService.handlePasswordRecovery(
        user.id,
        {
          ipAddress,
          userAgent,
  ...
  async recoverUser(cedula: string) {
    try {
      ...
      return {
        success: true,
        message: 'Se ha enviado la información al correo registrado',
      };
    } catch (error) {

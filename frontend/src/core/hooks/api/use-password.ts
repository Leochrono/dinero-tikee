...
export const usePassword = () => {
 ...
 const changePassword = async (currentPassword: string, newPassword: string, reason?: string) => {
  try {
    setLoading(true);
    ...
    if (reason === 'recovery') {
      ...
      const mappedReason = reason === 'recovery' ? 'reset' : 
                          reason === 'temporary_change' ? 'temporary_change' : 
                          'user_request';
  ...
  try {
    ...
    if (response.success) {
      ...
      navigate(routesWebpage.cambiarPassword, {
        ...
        replace: true // Añadido replace: true
      });
      return response;
    }
    ...
    setError(errorMessage);
    toast.error(errorMessage);
    console.error('Error in validateRecoveryCode:', error); // Debug
    return {
      success: false,
      error: errorMessage
    };
 ...
 const recoverPassword = async (email: string) => {
   try {
     ...
     if (response.success) {
       toast.success('Se ha enviado una contraseña temporal a tu correo electrónico');
       return true;
     }

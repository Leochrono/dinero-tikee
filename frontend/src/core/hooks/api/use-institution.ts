...
export const useInstitution = () => {
  ...
  const [lastFilters, setLastFilters] = useState<InstitutionFilters | null>(
    null
  );
  ...
  const filterInstitutions = useCallback(
    async (filters: InstitutionFilters): Promise<Institution[]> => {
      try {
        setLoading(true);
        setError(null);
        ...
        if (filteredInstitutions.length === 0) {
          const message = `No se encontraron instituciones que cumplan con los criterios. ${
            filters.amount
              ? `Prueba con un monto diferente a $${filters.amount.toLocaleString()}.`
              : ""
          }`;
          toast(message, {
            duration: 4000,
  ...
  const findBestRates = useCallback(
    async (params: BestRatesParams): Promise<Institution[]> => {
      try {
        setLoading(true);
        setError(null);

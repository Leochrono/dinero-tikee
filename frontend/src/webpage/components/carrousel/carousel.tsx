...
// Extender las definiciones de tipos de React
...
const Carousel: React.FC = () => {
  ...
  useEffect(() => {
    const imageLoadPromises = CAROUSEL_IMAGES.map(image => 
      new Promise<{src: string, loaded: boolean}>((resolve) => {
        const img = new Image();
        img.src = image.src;
        
        // Configuraciones de precarga
  ...
  // Memoización de la diapositiva activa
  const activeSlide = useMemo(() => {
    return CAROUSEL_IMAGES.find(img => 
      img.routes.includes(location.pathname)
    ) || CAROUSEL_IMAGES[0];
  }, [location.pathname]);
  ...
  // Crear un componente de imagen personalizado

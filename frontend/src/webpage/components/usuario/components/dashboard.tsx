...
const Dashboard = ({ credits }: DashboardProps) => {
  ...
  const summary = {
    ...
    byStatus: Object.keys(creditStatusConfig).reduce((acc, status) => {
      acc[status] = validCredits.filter(credit => credit.status === status).length;
      return acc;
    }, {} as Record<string, number>)
  };

  const orderedStatuses = Object.keys(creditStatusConfig)
    .sort((a, b) => creditStatusConfig[a as keyof typeof creditStatusConfig].order - 
        ...
        {orderedStatuses.map(status => (
          <Grid item xs={6} md={3} key={status}>
            <Box sx={{ 
              ...
              bgcolor: 'rgba(255, 255, 255, 0.05)', 
              borderRadius: 1,
              borderLeft: `4px solid ${creditStatusConfig[status as keyof typeof creditStatusConfig].color}`,
              textAlign: 'center'
            }}>
              <Typography variant="h4" sx={{ color: 'white' }}>
              ...
              <Typography variant="body2" sx={{ color: 'rgba(255, 255, 255, 0.7)' }}>
                {creditStatusConfig[status as keyof typeof creditStatusConfig].text}
              </Typography>
            </Box>
          </Grid>
        ))}
